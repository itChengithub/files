...
public class SeckillGoodsController {
    ...
    public void logIn(HttpServletRequest request){
        request.getSession().setAttribute("username","chenchao");
    }
    /**
     * 根据时间和秒杀商品ID查询秒杀商品数据
     */
    @GetMapping("/one")
    public Result<SeckillGoods> querySeckillGoods(String time,Long id){
        SeckillGoods seckillGoods=seckillGoodsService.findByTimeAndId(time,id);
        return new Result<SeckillGoods>(true,StatusCode.OK,"查询秒杀商品成功",seckillGoods);
    }
    /**
     * 查询秒杀时区
     */
    @GetMapping("/timeList")
    public Result<List<Date>> queryDateList(){
        List<Date> dateMenus = DateUtil.getDateMenus();
        return new Result<List<Date>>(true,StatusCode.OK,"查询秒杀时区成功",dateMenus);
    }
    /**
     * 获取参数时区下的所有秒杀商品
     * @param time
     */
    @GetMapping("/list")
    public Result<List<SeckillGoods>> queryTimeSeckillGoods(String time){
        List<SeckillGoods> seckillGoodsList=seckillGoodsService.findByTime(time);
        return new Result<List<SeckillGoods>>(true,StatusCode.OK,"查询当前时间内所有秒杀商品成功",seckillGoodsList);
    }
    /***
