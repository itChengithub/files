...
public class CategoryController {
    ...
    @GetMapping("/list/{pid}")
    public Result<List<Category>> findByPid(@PathVariable("pid") Integer pid){
        List<Category> categories = categoryService.findByPid(pid);
        return new Result<List<Category>>(true, StatusCode.OK,"查询子类型成功",categories);
    }
    @GetMapping("/brand/{id}")
    public Result<List<Brand>> findBrandById(@PathVariable("id") Integer id){
        return new Result<List<Brand>>(true,StatusCode.OK,"根据分类id查询品牌信息成功",categoryService.findBrandById(id));
    }
    ...
    public Result<Template> findTemplateById(@PathVariable("id") Integer id){
        return new Result<Template>(true,StatusCode.OK,"查询模板信息成功",categoryService.findTemplateById(id));
    }
    /***
     * Category分页条件搜索实现
     * @param category
